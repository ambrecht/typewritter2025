...
import WritingArea from '@/components/writing-area/writing-area';
...
import { measureCharWidth } from '@/utils/text-metrics';
...
export default function TypewriterPage() {
  ...
  const handleOrientationChange = useCallback(() => {
    ...
    if (lineBreakConfig.autoMaxChars && contentRef.current) {
      ...
      setTimeout(() => {
        ...
        const optimalLineLength = calculateOptimalLineLength(
          containerWidth,
          fontSize,
        );
  ...
  // ⬇️ direkt oberhalb des useLayoutEffect-Blocks einfügen
  const updateMaxChars = useCallback(() => {
    if (!lineBreakConfig.autoMaxChars || !contentRef.current) return;

    const lineWidthPx = contentRef.current.clientWidth;
    const charWidthPx = measureCharWidth('serif', fontSize);
    const optimal = Math.max(1, Math.floor(lineWidthPx / charWidthPx));

    if (optimal !== lineBreakConfig.maxCharsPerLine) {
      updateLineBreakConfig({ maxCharsPerLine: optimal });
    }
  }, [
    fontSize,
    lineBreakConfig.autoMaxChars,
    lineBreakConfig.maxCharsPerLine,
    updateLineBreakConfig,
  ]);

  const addLineAndRecalc = useCallback(
    (line: string) => {
      addLineToStack();
      updateMaxChars(); // sofort neue Grenze setzen
    },
    [addLineToStack, updateMaxChars],
  );

  useLayoutEffect(() => {
    // 1. einmal nach Font-Load
    (document.fonts?.ready ?? Promise.resolve()).then(updateMaxChars);

    // 2. Resize-Überwachung nur am relevanten Element
  ...
  return (
    <div
      ...
      className={`min-h-screen flex flex-col typewriter-container ${
        ...
        isAndroid ? 'android-typewriter' : ''
      } ${isFullscreen && isAndroid ? 'fullscreen' : ''} ${orientation}`}
      data-mode={mode}
      ...
      style={{
        ...
        ...(isAndroid && isKeyboardVisibleAndroid
    ...
    >
      ...
      <main
        className={`flex-1 flex flex-col p-4 md:p-6 lg:p-8 ${
      ...
      >
        <section
          ...
          className={`flex-1 flex flex-col ${
            darkMode
              ...
              : (isFullscreen ? 'bg-white' : 'bg-[#fcfcfa]') + ' shadow-md'
        ...
        >
          <WritingArea
            ...
            addLineToStack={addLineAndRecalc}
            ...
            isFullscreen={isFullscreen}
            linesContainerRef={
              linesContainerRef as React.RefObject<HTMLDivElement>
            }
          />
        </section>
      </main>
