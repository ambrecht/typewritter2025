...
const INITIAL_STATE: TypewriterState & {
  ...
  // Sound-Einstellungen
  soundEnabled: false, // Standardmäßig deaktiviert
  soundVolume: 0.8,
}
...
export interface TypewriterState {
  ...
  /** Zeitstempel der letzten Aktualisierung (für erzwungenes Neurendern) */
  _lastUpdated: number
}
  ...
  persist(
    (set, get) => ({
        ...
        set((state) => {
          ...
          if (lines.length > 1) {
            ...
            return {
              ...
              activeLineType: MarkdownType.NORMAL,
              ...paragraphInfo,
              _lastUpdated: Date.now(), // Erzwinge Neurendern
            }
          } else {
      ...
      updateLineBreakConfig: (config: Partial<LineBreakConfig>) => {
        console.log("Store: Updating lineBreakConfig with:", config)
        set((state) => {
          ...
          console.log("Store: Has changes:", hasChanges, "Current config:", state.lineBreakConfig)
      ...
      setFontSize: (size: number) => {
        const validSize = Math.max(9, Math.min(64, size || 24))
        console.log("Store: Setting fontSize to:", validSize)
      ...
      setStackFontSize: (size: number) => {
        const validSize = Math.max(9, Math.min(64, size || 16))
        console.log("Store: Setting stackFontSize to:", validSize)
        set(() => ({ stackFontSize: validSize }))
      },
        ...
        try {
          ...
          if (
            ...
            set({
              lastSaveStatus: {
                success: false,
                message: "API-Schlüssel ungültig oder nicht gefunden. Bitte überprüfen Sie Ihre Einstellungen.",
                timestamp: Date.now(),
              },
            })
          }
          ...
          if (result.success && result.text) {
            ...
            set({
              ...
              statistics: {
                ...
                pageCount:
                  Math.floor((result.letterCount || 0) / 1600) || calculateTextStatistics(result.text).pageCount,
              },
              ...
              lastSaveStatus: {
                success: true,
                message: `Sitzung vom ${new Date(result.createdAt || Date.now()).toLocaleString()} geladen`,
                timestamp: Date.now(),
              },
              _lastUpdated: Date.now(), // Erzwinge Neurendern
            })
            ...
            set({
              ...
              lastSaveStatus: {
                success: false,
                message: result.message || "Keine gespeicherte Sitzung gefunden",
                timestamp: Date.now(),
              },
            })
       ...
       */
      setSelectedLineIndex: (index: number | null) => set(() => ({ selectedLineIndex: index })),
        ...
        set((state) => {
          ...
          if (selectedLineIndex !== null) {
            ...
            if (selectedLineIndex >= lines.length - 1) {
              ...
              const newIndex = Math.min(lines.length - 1, selectedLineIndex + 1)
              return { selectedLineIndex: newIndex }
            }
          } else {
      ...
      navigateForward: (steps = 10) =>
        set((state) => {
          ...
          if (selectedLineIndex !== null) {
            ...
            const newIndex = Math.min(lines.length - 1, selectedLineIndex + steps)
